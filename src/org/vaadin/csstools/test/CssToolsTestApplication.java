...
public class CssToolsTestApplication extends Application {
	...
	public void init() {
		...
		Button get = new Button("Refresh Properties", new Button.ClickListener() {
			@SuppressWarnings("unchecked")
			public void buttonClick(ClickEvent event) {
				RenderInfo.get((Component) target.getValue(), new Callback() {
					public void infoReceived(RenderInfo info) {
						String text = "";
						for (CssProperty prop : CssProperty.values()) {
							props.get(prop).setValue(info.getProperty(CssProperty.valueOf(prop
									.toString())));
						}
					}
				});
